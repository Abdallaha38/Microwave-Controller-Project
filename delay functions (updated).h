...
void delay_us(int time){  // the argument is no. of micro-seconds i.e delay(5) it will generate a 5-micro-sec delay
   int i;
	 for(i=0;i< time; i++){
	 systick_wait_1us();
	 }	 
...
void systick_wait_1ms(){
   NVIC_ST_RELOAD_R = 16000 - 1;     //frequency is 16MGHz
	 NVIC_ST_CURRENT_R = 0;
	 while (( NVIC_ST_CTRL_R & 0x00010000) == 0){}
}

// Function that makes multiple of millie seconed delay
void delay_ms(int time){  // the argument is no. of m-seconds i.e delay(5) it will generate a 5-m-sec delay
   int i;
	 for(i=0;i< time; i++){
	 systick_wait_1ms();
	 }	 
